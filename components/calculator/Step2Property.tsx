      ...
      <div className="space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex text-xs bg-[#0f3460] rounded-lg overflow-hidden border border-[#1e3a5f]">
              <button
                type="button"
                onClick={() => setDepositMode('amount')}
                className={`px-3 py-1 transition-colors ${depositMode === 'amount' ? 'bg-[#3b82f6] text-white' : 'text-[#94a3b8] hover:text-white'}`}
              >
                {sym} Amount
              </button>
              <button
                ...
                className={`px-3 py-1 transition-colors ${depositMode === 'percent' ? 'bg-[#3b82f6] text-white' : 'text-[#94a3b8] hover:text-white'}`}
              >
                % Percent
              </button>
            </div>
          ...
          {depositMode === 'amount' ? (
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-4 pr-8 py-3 bg-[#0f3460] border border-[#1e3a5f] rounded-lg text-white placeholder-[#94a3b8] focus:outline-none focus:border-[#3b82f6] transition-colors"
                placeholder="20"
                min={0}
                max={100}
                step={0.5}
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs text-[#94a3b8]">
              Loan: {sym}{loanAmount.toLocaleString()} at <span className="font-semibold text-white">{(ltv * 100).toFixed(1)}% LTV</span>
            </span>
            <span className={`text-xs px-2 py-0.5 rounded-full ${
              ltv <= 0.6 ? 'bg-green-900/40 text-green-400' :
              ltv <= 0.8 ? 'bg-blue-900/40 text-blue-400' :
              ltv <= 0.9 ? 'bg-amber-900/40 text-amber-400' :
        ...
        <div>
          ...
          <label className="flex items-center gap-2 mt-2 cursor-pointer">
            <input
              ...
              className="rounded border-[#1e3a5f] bg-[#0f3460] accent-[#3b82f6]"
            />
            <span className="text-xs text-[#94a3b8]">Roll these fees into the mortgage</span>
          </label>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-[#0f3460] border border-[#1e3a5f] rounded-lg text-white focus:outline-none focus:border-[#3b82f6] transition-colors"
          />
        </div>
