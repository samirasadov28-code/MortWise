  ...
  return (
    <div className="bg-white border border-[#e8e3dc] rounded-xl p-5">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h3 className="text-sm font-semibold text-[#2a2520]">{t('repayment.title')}</h3>
        {results.length > 1 && (
          <select
            value={selectedIdx}
      ...
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          <Area type="monotone" dataKey="principal" stackId="1" stroke="#4a7c96" fill="#4a7c96" fillOpacity={0.4} name={t('repayment.principal')} />
          <Area type="monotone" dataKey="interest" stackId="1" stroke="#ef4444" fill="#ef4444" fillOpacity={0.3} name={t('repayment.interest')} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
